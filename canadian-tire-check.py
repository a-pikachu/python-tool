...
def search_and_scrape_first_card(page, search_query):
    ...
    # Case 1: Explicit out-of-stock string
    if "out of stock" in lower:
        return "Out of stock", 0

    # Case 2: Extract numeric stock value
    m = re.search(r"(\d+)", stock_text)
    if m:
        quantity = int(m.group(1))
        return f"{quantity} In Stock", quantity
    ...
    with sync_playwright() as p:
        ...
        for product in PRODUCTS:
            ...
            for store_label, search_query in STORES.items():
                ...
                print(f"{store_label}: {store_name} → {quantity} In Stock")
